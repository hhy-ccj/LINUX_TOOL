#!/bin/sh
filter_path="! -path "*br17*" ! -path "*ac461x_uboot_lib*" ! -path "*bt16*""
...
find $filter_path -regex '.*\.\(c\|h\)' > cscope.input
...
find . -not -regex '.*\.\(png\|gif\|o\|d\|obj\|swp\)' $filter_path ! -path "*git*" -type f -printf "%f\t%p\t1\n" | sort -f>>filenametags
